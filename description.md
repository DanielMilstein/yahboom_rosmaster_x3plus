...
| Base doesn't move on hardware | Executor started without the launch file's topic overrides (§7.1), or `enable_base_drive:=false`. On bridge builds before 2026-06-12, `/cmd_vel` shared a callback group with the overrunning joint-state poller and was starved — rebuild the bridge. |
| Base drives too fast and/or overshoots into obstacles | Wrong `car_type` enum. It scales commands AND odometry feedback (1/4 invert the reported sign → the closed loop becomes positive-feedback runaway). `car_type=2` is sweep-verified for this X3 Plus (`scripts/probe_car_type.py` re-derives it). The executor's `drive_abort_divergence_m` guard stops a runaway within ~10 cm. |
