...
int main(int argc, char **argv) {
    arm_moveit_demo::ScopedMoveItNode demo(argc, argv, "moveit_revise_trajectory_demo");
    const auto logger = demo.node()->get_logger();

    moveit_msgs::msg::RobotTrajectory trajectory;
    moveit::planning_interface::MoveGroupInterface yahboomcar(demo.node(), "arm_group");
    moveit_visual_tools::MoveItVisualTools tool(demo.node(), yahboomcar.getPlanningFrame());
    ...
    for (const auto& pose : poses) {
        multi_trajectory(yahboomcar, pose, trajectory);
    }
